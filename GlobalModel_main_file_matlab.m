...
if parametermapplot
    if ~isempty(figures)
        ...
        if isempty(fig20)
            ...
            clearvars -except R0_PS numSurfaces surfhandles surflabels parametermapplot
        end
...
pybamm=false; %use the orginal parameters or stuff from pybamm
...
close all
% Chen_Marquis_diffusivity_concentration_radius_mod_OCV_Marquis, Chen_OCV_Marquis
% Chen_OCV_Marquis_diffusivity_conc_rad
titletext='';
k_PS=1;
...
start=1; %Start time 
endglobalmodel=4;%*5000; number of chunks
fs = 20;
...
TempRange = './Parameters_Temp'; %ATT: noSOC: './Parameters_Temp_noSOC'
files1 = dir(fullfile(TempRange, 'Parameters1RC*.mat')); % ATT: for noSOC: 'Parameters1RC_noSOC_*.mat'
if pybamm %&& parametermapplot
...
OCV_TempRange= './OCV_SOC_Temp/OCV_SOC_25degrees.mat'; 
%OCV_TempRange= './Results_CHEN/OCV_SOC_Temp'; %ATT

files2 = dir(fullfile(OCV_TempRange));
...
for i = 1:length(files1)%filesofinterest%:7%1:length(files1) %5:25°C  
    filePath1 = fullfile(TempRange, files1(i).name)
    filePath2 = fullfile(OCV_TempRange) %always use the
...
if ~pybamm
    ...
    for V_ECM_Temp_idx=V_ECM_Temp_nonempty'
        plot(t(start:endglobalmodelplot)/3600, V_ECM_Temp{V_ECM_Temp_idx}, 'LineWidth', 2)
        legendStr{end + 1} = [Temperatures{V_ECM_Temp_idx} '$^{\circ}$C'];
        %legend(legendStr, 'interpreter', 'latex', 'Location', 'southeast');
    end
    legendStr{end + 1} = ['DFN'];
    plot(t(start:endglobalmodelplot)/3600, all_Volt(start:endglobalmodelplot), 'LineWidth', 2,'Color','k')
    legend(legendStr, 'FontSize', fs,'interpreter', 'latex', 'Location', 'southeast');
    ax=gca;
    ax.FontSize = fs;
    ax.XTick = [];
    % xlim([0,17000]/3600)
    ...
    subplot(3,3,7)
    plot(t(start:endglobalmodelplot)/3600, all_I(start:endglobalmodelplot)/OneC, 'LineWidth', 2)
    ...
    subplot(3,3,8)
    plot(t(start:endglobalmodelplot)/3600, all_T(start:endglobalmodelplot)-273.15, 'LineWidth', 2)
    ...
    subplot(3,3,9)   
    plot(t(start:endglobalmodelplot)/3600, all_SOC(start:endglobalmodelplot), 'LineWidth', 2)
...
i_start=3; %att: 3 usually 
for i=i_start:length(Params_c)
    ...
    if pybamm_shift
        ...
        if pybamm_normalize
            ...
            surf(current_cd/OneC,SOC,R0_cd*1000,'FaceColor',facecolorvalue,'FaceAlpha',FaceAlpha_value);      
...
% figure
% plot(SOC,tau_cell{5},'LineWidth', 2,'Color','b')
% xlabel('SOC', 'FontSize', fs*2, 'interpreter', 'latex');
% ylabel('$\tau_1$', 'FontSize', fs*2, 'interpreter', 'latex');
% ylim([0,30])
% ax=gca;
% ax.FontSize = fs;
...
%legend([h1, h2], {'Noise: 5\% standard deviation of voltage', 'Noise: 10\% standard deviation of voltage'},'FontSize',fs,'Interpreter','latex');

legend([h1, h2, h3], {'$\Delta R_0$ Noise: 5\% standard deviation of voltage', '$\Delta R_0$ Noise: 10\% standard deviation of voltage','Default parameter map'},'FontSize',fs,'Interpreter','latex');
%%
...
if include_voltage_plot
    %nexttile(1, [2 2]);
    nexttile(1, [2 3]);
    plot(t(start:end)/3600, all_Volt(start:end), 'LineWidth', 2, 'Color', 'r');
    hold on
    %plot(t(start+1:end), V_ECM_Temp{5}(1:end-1), 'LineWidth', 2,'Color','b');
    plot(t(start+1:end)/3600, V_ECM_Temperature_interp(1:end-1), 'LineWidth', 2,'Color','b','LineStyle','--');
    plot(t(start+1:end)/3600, V_ECM_Temp{5}(1:end-1), 'LineWidth', 2,'LineStyle','-.');

   
    ylabel('Voltage (V)', 'FontSize', fs, 'interpreter', 'latex');
    xlabel('Time (h)', 'FontSize', fs, 'interpreter', 'latex');
    legend('DFN','ECM (parameters temperature adjusted)','ECM (parameters for 25$^\circ$C)', 'interpreter', 'latex', 'Location', 'southwest');
    ax=gca;
    ax.FontSize = fs;
    ax.XTick = [0:1:5];
    xlim([0,17000]/3600)
...
if include_current_plot
    nexttile(7);
    plot(t(start:end)/3600, all_I(start:end)/OneC, 'LineWidth', 2, 'Color', 'b');
    ylabel('C-rate', 'FontSize', fs, 'interpreter', 'latex');
    xlabel('Time (h)', 'FontSize', fs, 'interpreter', 'latex');
    xlim([0,(17000/3600)])
    ylim([-1,1])
...
if include_temperature_plot
    nexttile(8);
    plot(t(start:end)/3600, all_T(start:end)-273.15, 'LineWidth', 2, 'Color', 'b');
    ylabel('Temp. ($^{\circ}$C) ', 'FontSize', fs, 'interpreter', 'latex');
    xlabel('Time (h)', 'FontSize', fs, 'interpreter', 'latex');
    xlim([0,(17000/3600)])
    ax=gca;
...
if include_SOC_plot
    nexttile(9);
    plot(t(start:end)/3600, all_SOC(start:end), 'LineWidth', 2, 'Color', 'b');
    ylabel('SOC', 'FontSize', fs, 'interpreter', 'latex');
    xlabel('Time (h)', 'FontSize', fs, 'interpreter', 'latex');
    xlim([0,(17000/3600)])
    ax=gca;
...
ax = findobj(gcf,'Type','Axes');
linkaxes(ax, 'x');
%xlim([8942,18909]/3600)
%ylim([3,4.7])

box on
ax.FontSize = fs;
set(2, 'units', 'centimeters', 'pos', [0 0 35 20])
exportgraphics(figure2,'Temp combined_V2.pdf')
%%
